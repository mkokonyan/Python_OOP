...
from helper_classes import PersonWithDunders, PersonWithoutDunders
...
class TestCustomList(TestCase):
    ...
    def test_extend_appends_iterable_to_values_with_empty(self):
        cl = CustomList()
        self.assertEqual([], cl._CustomList__values)
        cl.extend([1, 2])
    ...
    def test_extend_returns_new_list_and_modifies_old(self):
        self.assertEqual([1, 2, 3], self.custom_list._CustomList__values)
    ...
    def test_sum_raises_if_object_does_not_implement_dunder_add(self):
        ...
        with self.assertRaises(ValueError) as ex:
            cl.sum()
        self.assertEqual("All object must implement dunder add", str(ex.exception))
    ...
    def test_overbound_raises_if_object_does_not_implement_dunder_len(self):
        ...
        with self.assertRaises(ValueError) as ex:
            cl.overbound()
        self.assertEqual("All object must implement dunder len", str(ex.exception))
