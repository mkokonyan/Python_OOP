class PizzaDelivery:
    ...
    def make_order(self):
        self.ordered = True
        ingredients_result = [f"{i}: {q}" for i, q in self.ingredients.items()]
        return f"You've ordered pizza {self.name} prepared with {', '.join(ingredients_result)} and the price will be {self.price}lv."
